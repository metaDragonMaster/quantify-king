<template>
    <div class="explore limit-max-width-media">
                ...
                >
                    ...
                    <div class="info theme-box-shadow">
                        ...
                        <div class="cell">
                            ...
                            <span
                                class="font-o color-green"
                            >{{ textFromWei(userInfo.interest_bearing_principal) }}</span>
                        </div>
                        <div class="cell font-o">
                            <span class="color-grey">USDT ( ERC20.Ethereum )</span>
                            <span>~ ￥{{ textFromWei(userInfo.interest_bearing_principal) * 6.3 }}</span>
                        </div>
                    </div>
                </li>
                <li
        ...
        <section class="model-5">
            ...
            <img src="@/assets/explore/m-5.png" alt />
            <div class="cell">
                <div class="color-grey">
                    ...
                    <p>
                        <img src="@/assets/icons/icon-reserved.png" alt />
                        <span>uFerce AII Rightx Reserved.</span>
                    </p>
                </div>
                <Link to="/explore">
                    <button class="theme-button-clip font-o button">联系我们 ---></button>
                </Link>
            </div>
        </section>
    </div>
...
async function join() {
    ...
    try {
        ...
        const res = await QKContract.methods.join(AbiAddressUSDT).send({
...
async function getUserInfo() {
    ...
    try {
        ...
        const res = await QKContract.methods.get_user_info(AbiAddressUSDT, userAddress.value).call();
...
const earnEveryTime = computed(() => earning.interest / earning.second || 'Interest bearing...')

async function startEarning() {
    earning.Load = true;
    try {
        const res = await calculateEarnings(userInfo.interest_bearing_principal);
        earning.second = res[0];
        earning.interest = web3.value.utils.fromWei(res[1]);
...
const userCalcEveryTime = computed(() => userCalcData.interest / userCalcData.second || 'Interest bearing...')
...
async function withdraw() { //提现
    const load = lockLoadHandler('loading...')
    try {
        const { QKContract } = Contracts.value
        const res = await QKContract.methods.withdraw(
            AbiAddressUSDT,
            userAddress.value,
...
async function getKingSort() {
    ...
    try {
        ...
        const reverseRes = deepClone(res).reverse().map(item => {
