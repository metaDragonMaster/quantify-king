...
async function getResChild(id, func) {
  ...
  try {
    ...
    const res = await QKContract.methods[func](userAddress.value).call();
    console.log(res);
    tableData.value = res;
...
const weiIncome = computed(() => textFromWei(reInterests._income));
const myInterests = computed(() =>
  textFromWei(
    (Number(reInterests._income) + Number(reInterests._released)).toString()
  )
);
async function getReInterests() {
  try {
    ...
    const res = await QKContract.methods
      .get_re_interests(AbiAddressUSDT, userAddress.value)
      .call();
    /*
            _income: "0" 可领取
            _released: "0" 已领取
...
async function getUserAllReBalanceOf(id) {
  ...
  try {
    const { QKContract } = Contracts.value;
    const res = await QKContract.methods
      .get_user_All_re_BalanceOf(AbiAddressUSDT, userAddress.value, id)
      .call();
    console.log(res);
...
async function reWithdraw() {
  const load = lockLoadHandler("receiving...");
  try {
    const { QKContract } = Contracts.value;
    const res = await QKContract.methods.re_withdraw(AbiAddressUSDT).send({
